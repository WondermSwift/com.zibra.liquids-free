...
namespace com.zibra.liquid.SDFObjects
{
    ...
    /// </summary>
    [AddComponentMenu("Zibra/Analytic SDF Collider")]
    public class AnalyticSDFCollider : SDFCollider
    {
        ...
        {
            if (isInitialized.Contains(InstanceID)) return;
        ...
        private void ComputeSDF_Base(int InstanceID, CommandBuffer SolverCommandBuffer, ComputeBuffer Coordinates, ComputeBuffer Output,
            ComputeBuffer OutputID, int ID, int Elements, CalculationType type)
        {
            ...
            SolverCommandBuffer.IssuePluginEventAndData(ZibraLiquidBridge.RunSDFShaderWithDataPtr(), ZibraLiquidBridge.EventAndInstanceID(7, InstanceID), NativeDataPtr);
        }
        ...
        public override void ComputeSDF_Unite(int InstanceID, CommandBuffer SolverCommandBuffer, ComputeBuffer Coordinates,
            ComputeBuffer Output, ComputeBuffer OutputID, int ID, int Elements)
        {
            ComputeSDF_Base(InstanceID, SolverCommandBuffer, Coordinates, Output, OutputID, ID, Elements, CalculationType.Unite);
        }
    }
}